...
export class DbUtil {
  ...
  queryForList<T>(predicates: relationalStore.RdbPredicates, columns: ColumnInfo[]): Promise<ScheduleModel[]> {
    return new Promise((resolve, reject) => {
  ...
  parseResultSet<T> (result: relationalStore.ResultSet, columns: ColumnInfo[]): ScheduleModel[] {
    // 1.声明最终返回的结果
    let arr = []
    // 2.判断是否有结果
